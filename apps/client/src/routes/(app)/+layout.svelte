<script lang="ts">
	...
	let appcolor_timeout: NodeJS.Timeout;
	...
	onMount(async () => {
		try {
			...
			window.SetColored = (isColored: 'default' | 'green' | 'red') => {
				clearTimeout(appcolor_timeout);
				appcolor = isColored;
				appcolor_timeout = setTimeout(() => {
					appcolor = 'default';
				}, 1500);
			};
			...
			setInterval(() => {
				window.navigator.onLine ? (appstate = 'app') : (appstate = 'offline');
			}, 1000);
	...
	function setTitle() {
		...
		const currentCollection = collections.find((c) => c.id === currentPath.split('/')[1]);
		title = currentCollection?.name ? `${currentCollection.name} • Dotpen` : title;
